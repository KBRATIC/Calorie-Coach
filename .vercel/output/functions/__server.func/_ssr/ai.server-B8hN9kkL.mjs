//#region node_modules/.nitro/vite/services/ssr/assets/ai.server-B8hN9kkL.js
var SYSTEM = `Você é nutricionista e converte descrições de refeições em português (Brasil) para JSON.
...
function normalizeItems(items, defaultMeal) {
...
function parseJsonContent(content, defaultMeal) {
	let parsed;
	try {
	...
	return normalizeItems(Array.isArray(parsed.items) ? parsed.items : [], defaultMeal);
}
async function parseWithGoogle(text, defaultMeal, key) {
	const url = new URL("https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-lite-latest:generateContent");
	...
	return parseJsonContent(json.candidates?.[0]?.content?.parts?.[0]?.text ?? "{}", defaultMeal);
}
async function parseMealText(text, defaultMeal) {
